...
export default function App({ navigate, route }) {
  ...
  const fetchAllContent = async () => {
    try {
      ...
      if (data.lawyers.success) setLawyers(data.lawyers.data || []);
      if (data.practiceAreas.success) setPracticeAreas(data.practiceAreas.data || []);
      if (data.publications.success) setPublications(data.publications.data || []);
      if (data.clientLogos.success) setClientLogos(data.clientLogos.data || []);
      if (data.journals.success) setJournals(data.journals.data || []);
      if (data.insights.success) setInsights(data.insights.data || []);
      if (data.articles.success) setArticles(data.articles.data || []);
      if (data.aboutContent.success) setAboutContent(data.aboutContent.data || []);
      if (data.whyChooseContent.success) setWhyChooseContent(data.whyChooseContent.data || []);
      if (data.consultationFeesContent.success) setConsultationFeesContent(data.consultationFeesContent.data || []);
      if (data.heroContent.success && data.heroContent.data && data.heroContent.data.length > 0) setHeroContent(data.heroContent.data[0]);
      if (data.heroBanners?.success) setHeroBanners(data.heroBanners.data || []);
      if (data.featureStrips.success) setFeatureStrips(data.featureStrips.data || []);
      if (data.siteSettings?.success) setSiteSettings(data.siteSettings.data || {});
    } catch (err) {
  ...
  const submitConsultation = async (e) => {
    ...
    try {
      ...
      const data = await api.submitConsultation(payload);
      if (!data?.success) {
