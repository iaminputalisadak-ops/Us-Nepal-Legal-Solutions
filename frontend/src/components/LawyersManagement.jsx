...
export default function LawyersManagement({ token: tokenProp }) {
  ...
  const fetchLawyers = async () => {
    try {
      const data = await api.getLawyersWithAuth(getToken());
      if (data.success) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingLawyer) payload.id = editingLawyer.id;

      const data = editingLawyer
        ? await api.updateLawyer(payload)
        : await api.createLawyer(payload);
