...
export default function ContentManager({ contentType, token: tokenProp }) {
  ...
  const fetchItems = async () => {
    try {
      const data = await api.getContent(contentType);
      if (data.success) {
        setItems(data.data || []);
      }
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = editingItem
        ? await api.updateContent(contentType, payload)
        : await api.createContent(contentType, payload);
      if (data.success) {
        await fetchItems();
        resetForm();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const data = await api.deleteContent(contentType, id);
      if (data.success) {
        await fetchItems();
        try {
          localStorage.setItem("siteContentUpdatedAt", String(Date.now()));
