...
export default function ImageField({
  ...
  const doUpload = async (file) => {
    ...
    if (!token) throw new Error("Not logged in. Please logout and login again.");

    const fd = new FormData();
    fd.append("image", file);
    // Send token as form field for maximum compatibility with PHP/Apache setups
    fd.append("token", token);

    const res = await fetch(`${API_URL}/upload.php`, {
      method: "POST",
      // Avoid relying on Authorization header; some setups strip it.
      body: fd,
    });
